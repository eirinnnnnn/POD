...
int main(int argc, char const *argv[]){
    ...
    config["OSD"]["Gmatrix_path"] = "../../../data/BCH_51_64_g.matrix";
    config["OSD"]["Hmatrix_path"] = "";
    config["OSD"]["matrix_src"] = "byGmatrix";
    config["OSD"]["OSD_order"] = "3";
    ...
    ErrorCorrectionCodeBase *ECC_OSD = new OSD(config["OSD"]);
    channelBase *channel_AWGN = new AWGN(config["AWGN"]);
    channel_AWGN->setCodeRate(ECC_OSD->getCodeRate());
    // channel_AWGN->setCodeRate(7.0/15.0);

    std::vector<char> message(ECC_OSD->getMessageLength());
    std::vector<char> codeword(ECC_OSD->getCodewordLength());
    std::vector<double> received(ECC_OSD->getCodewordLength());
    std::vector<char> decoded_word(ECC_OSD->getCodewordLength());
    ...
    while(do_monte_carlo){
        iter_count++;
        // random message
        for(unsigned int i_idx=0;i_idx<ECC_OSD->getMessageLength();i_idx++)
        ...
        // doEncode
        ECC_OSD->doEncode(message,codeword);
        // printf("codeword ");
        // for(unsigned int i_idx=0; i_idx<codeword.size(); i_idx++)
        //  printf("(%d)",codeword[i_idx]);
        // printf("\n");
        // all zero codeword
        // codeword.assign(ECC_OSD->getCodewordLength(), 0);
        // channel
        channel_AWGN->addNoise(codeword,received);
        for(unsigned int idx=0; idx<ECC_OSD->getCodewordLength(); idx++){
        ...
        // doDecode
        ECC_OSD->doDecode(received,decoded_word,decode_log);
        // printf("decoded   ");
        ...
        if(iter_count % monitor_slot_size == 0){
            printf("%s, BLER = %8d/%8d = %8.6f, BER = %8.6f, RBER = %8.6f\r",
               ...
               (double)bit_error_count/iter_count/ECC_OSD->getCodewordLength(),
               (double)noise_bit_count/iter_count/ECC_OSD->getCodewordLength()
               );
            fflush(stdout);
        }
        // exit(0);
