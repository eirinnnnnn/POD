...
static void usage() {
    std::cout
        ...
        << "  --target-errors N            Stop early once N full-ensemble block errors\n"
        << "                                (no branch correct) are observed. --samples\n"
        << "                                then acts as the safety cap on total trials.\n";
}
...
static Args parseArgs(int argc, char **argv) {
    ...
    for(int i=1; i<argc; i++) {
        ...
        } else if(a == "--target-errors") {
            args.target_errors = (unsigned int)std::stoul(need(a));
        } else if(a == "--out") {
            args.out_csv = need(a);
        } else if(a == "--snr") {
            args.snr = need(a);
...
static void writeMetadata(const Args &args,
    ...
    meta << "  \"samples\": " << actual_samples << ",\n";
    meta << "  \"max_samples\": " << args.samples << ",\n";
    meta << "  \"target_errors\": " << args.target_errors << ",\n";
    meta << "  \"block_errors\": " << block_errors << ",\n";
    ...
    try {
        ...
        for(unsigned int sample=0; sample<args.samples; sample++) {
            ...
            for(unsigned int b=0; b<M; b++) {
                ...
                any_correct = any_correct || branches[b].correct;
            }
            ...
            if(args.target_errors > 0 && block_errors >= args.target_errors)
                break;
        }

        writeMetadata(args, config, decoder, actual_samples, block_errors);
        std::cout << "wrote " << args.out_csv << "\n";
        std::cout << "wrote " << metadataPath(args.out_csv) << "\n";
        std::cout << "samples=" << actual_samples << " block_errors=" << block_errors
                   << " n=" << n << " k=" << k << " M=" << M << "\n";
        return 0;
