#!/usr/bin/env python3
"""Low-SNR refine pass for the t_sweep_m8_PED_baseline trial: SNR 2.00-3.00dB
only, target-errors bumped 500->1000 to smooth out the noisiest, cheapest
region of the curve. Reuses the same M set (8,16,17,19,24,36). Unlike the
main driver's append_rows, this REPLACES any existing row for the same
(snr, M) key rather than skipping it, so the higher-precision numbers
overwrite the old 500-error ones in t_sweep_m8_PED_baseline_results.txt.
Meant to be run only after the main sweep/baseline trials finish, to avoid
competing for CPU."""
import csv
import subprocess
...
def main():
    for i, snr in enumerate(SNR_LIST):
        ...
        for L, M in L_TO_M.items():
            ...
            log = OUTDIR / f"t_sweep_ped_M{M}_snr{t_tag}_refine1000.log"
            ...
            ], f"t_sweep_ped_M{M}_snr{t_tag}_refine1000")
            replace_rows([{"snr": snr, "L": L, "M": M, "method": "ped_m_scl4",
                           "samples": vals["samples"], "errors": vals["full_ped_errors"],
                           "bler": vals["full_ped_bler"]}])
            print(f"SNR={snr} M={M} (L={L}) refined (cached={cached}, samples={vals['samples']:.0f})")
