#!/usr/bin/env python3
"""BLER-vs-Eb/N0 plot for the Golay24 M=16/t=12 checkpoint-ranked
static-pm-rank sweep: full-PED (M=16) vs static-pm-rank at m=1,4,8,12,
...
RESULTS = HERE / "sweep" / "golay_m16_t12_results.txt"
...
def main():
    ...
    for m in M_LIST:
        ...
        ax.plot([s for s, _ in pts], [b for _, b in pts], color=COLORS[m],
                 ...
                 label=f"static pm-rank (t=12, m={m})", zorder=2)

    ax.set_yscale("log")
    ax.set_xlabel("Eb/N0 (dB)")
    ax.set_ylabel("BLER")
    ax.set_title("Golay(24,12) AED (M=16, list_size=4, checkpoint t=12): full-PED vs static-pm-rank")
    ax.grid(True, which="both", alpha=0.3)
    ax.legend(fontsize=8, ncol=2, loc="lower left", framealpha=0.9)
    fig.tight_layout()

    out = HERE / "golay_m16_t12_bler.png"
