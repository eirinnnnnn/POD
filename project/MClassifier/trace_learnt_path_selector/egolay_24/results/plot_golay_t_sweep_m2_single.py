#!/usr/bin/env python3
"""BLER-vs-Eb/N0 plot for the Golay24 t-sweep trial: fixed m=2, checkpoint
t in {4,8,16,20}, static-pm-rank vs trace-learned, vs full-PED (M=16,
reused from the fixed-t=8 trial) and pure-SCL(L=8)/pure-SC(L=1) baselines,
...
RESULTS = HERE / "sweep" / "golay_t_sweep_m2_single_results.txt"
...
def main():
    ...
    ax.set_ylabel("BLER")
    ax.set_title("Golay(24,12) AED (M=16, list_size=2, m=2): full-PED vs static-pm-rank vs trace-learned over t")
    ax.grid(True, which="both", alpha=0.3)
    ax.legend(fontsize=7, ncol=2, loc="lower left", framealpha=0.9)
    fig.tight_layout()

    out = HERE / "golay_t_sweep_m2_single_bler.png"
