#!/usr/bin/env python3
"""BLER-vs-Eb/N0 plot for the Golay24 SC-base/M=16 trial: full-PED (M=16,
SC base) vs static-pm-rank at m=1,4,8,12, vs pure-SCL(L=8, rough MLD
reference) and pure-SC (weak-decoder floor), over SNR=[0.0, 0.5, 3.0]."""
import csv
...
RESULTS = HERE / "sweep" / "golay_sc_m16_results.txt"
...
def main():
    ...
    full_ped, static, pure_scl8, pure_sc = defaultdict(list), defaultdict(list), [], []
    for r in rows:
    ...
    if pts:
        ax.plot([s for s, _ in pts], [b for _, b in pts], color="#2a78d6",
                 ...
                 label="full PED (M=16, SC base)", zorder=3)
    ...
    if pts:
        ax.plot([s for s, _ in pts], [b for _, b in pts], color="#333333",
                 ...
                 label="pure SCL (L=8, rough MLD ref)", zorder=2)
    ...
    ax.set_ylabel("BLER")
    ax.set_title("Golay(24,12) AED (SC base, M=16): full-PED vs static-pm-rank")
    ax.grid(True, which="both", alpha=0.3)
    ax.legend(fontsize=8, ncol=2, loc="lower left", framealpha=0.9)
    fig.tight_layout()

    out = HERE / "golay_sc_m16_bler.png"
