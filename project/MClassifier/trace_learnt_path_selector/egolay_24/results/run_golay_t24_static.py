#!/usr/bin/env python3
"""Golay24 static-pm-rank sanity check at t=24 (the full codeword length,
n=24): ranking by the checkpoint captured at the very last decode position
should collapse to ranking by the FINAL metric, i.e. static_pm_rank(t=24)
should collide with full_ped -- this is the same tautology that made the
original Phase-1 static rank incapable of diverging from full-PED, now
checked deliberately as a sanity point rather than a bug. No --model is
passed (static_pm_rank is computed unconditionally by the sim, independent
of any trace-learned model). Rows are appended to BOTH the _single and
_cumulative results files under t=24, since static_pm_rank at a given
checkpoint doesn't depend on which model file (if any) happens to be
loaded alongside it."""
import csv
import subprocess
...
def seed_pair(snr_idx):
    return 960001 + snr_idx, 860001 + snr_idx
...
def main():
    ...
    for i, snr in enumerate(SNR_LIST):
        ...
        cap = SAMPLE_CAP[snr]
        cs, ms = seed_pair(i)
        log = OUTDIR / f"golay_t_sweep_m2_t24_snr{t_tag}.log"
        vals, cached = run_sim(log, [
            ...
            "--m-list", str(M),
            "--monitor-every", "50000",
        ], f"golay_t_sweep_m2_t24_snr{t_tag}")
        ...
        print(f"SNR={snr} t=24 done (cached={cached}, samples={samples:.0f})")
