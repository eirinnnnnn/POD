...
def main():
    parser = argparse.ArgumentParser(description="Predict top-k branches from a metric-soft ranker.")
    ...
    with torch.no_grad():
        scores = model(torch.from_numpy(Xn.astype(np.float32)))
        topk = torch.topk(scores, k=min(args.topk, int(ckpt["M"])), dim=1).indices.cpu().numpy()
