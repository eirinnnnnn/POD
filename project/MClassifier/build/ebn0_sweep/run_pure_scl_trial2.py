#!/usr/bin/env python3
"""Pure-SCL (no AED ensemble) diversity trial, L in {65, 68, 76, 96} --
list sizes comparable to the M=64 AED branch count, using the bug-fixed
mclass_bler_sim (the use_AED override that invalidated trial 1's pure_scl
numbers is fixed) and the clean automorphism file. Same SNR grid / sample
caps as run_clean_sweep.py so results are directly comparable to that run's
full-PED/static/trace curves.
"""
import csv
import subprocess
...
RESULTS = OUTDIR / "t_sweep_m8_SCL_baseline_results.txt"
...
def main():
    for i, snr in enumerate(SNR_LIST):
        ...
        for L in L_LIST:
            ...
            log = OUTDIR / f"pure_scl2_L{L}_snr{t_tag}.log"
