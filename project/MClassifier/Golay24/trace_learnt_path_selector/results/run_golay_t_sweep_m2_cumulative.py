#!/usr/bin/env python3
"""Golay24 t-sweep trial (CUMULATIVE checkpoint variant): fixed pruning
width m=2, checkpoint t in {4,8,16,20}, static-pm-rank vs trace-learned
(one model per t, each trained at SNR=2.0dB on CUMULATIVE features -- the
t model sees stats from ALL checkpoints <= t concatenated, eBCH's
"upto_tX" convention -- see golay_train_t{T}_cum_12k.csv /
golay_trace_t{T}_cum_weights.txt). This is the direct counterpart of
run_golay_t_sweep_m2_single.py, which used single-checkpoint-only models.
Same 8-point SNR grid, target_errors=1000, same sample caps. full_ped
(M=16) and the pure-SCL(L=8)/pure-SCL(L=2)/pure-SC(L=1) no-AED baselines
are checkpoint-independent, so they're reused directly from the _single
trial's results rather than recomputed."""
import csv
import subprocess
...
RESULTS = OUTDIR / "golay_t_sweep_m2_cumulative_results.txt"
...
def seed_pair(snr_idx, salt):
    return 860001 + salt * 100 + snr_idx, 760001 + salt * 100 + snr_idx
...
def import_reference_curves():
    """Reuse full_ped + no-AED baselines from the _single trial (all checkpoint-independent)."""
...
def main():
    ...
    for T in T_LIST:
        model = BUILD / f"golay_trace_t{T}_cum_weights.txt"
        for i, snr in enumerate(SNR_LIST):
            t_tag = tag(snr)
            cap = SAMPLE_CAP[snr]
            cs, ms = seed_pair(i, T)
            log = OUTDIR / f"golay_t_sweep_m2_cum_t{T}_snr{t_tag}.log"
            vals, cached = run_sim(log, [
                ...
                "--model", str(model),
                "--monitor-every", "50000",
            ], f"golay_t_sweep_m2_cum_t{T}_snr{t_tag}")
