...
def load_trace_dataset(path, branch_onehot=True):
    # A plain-numeric np.loadtxt pass is dramatically lighter than
    # np.genfromtxt's structured-dtype parser on multi-GB trace CSVs (tens of
    # MB of peak memory vs. many GB) -- column names are recovered from the
    # header line separately instead of relying on genfromtxt's names=True.
    ...
    samples = raw[:, col["sample"]].astype(np.int64)
    branches = raw[:, col["branch"]].astype(np.int64)
    ...
    raw = raw[order]
    branches = raw[:, col["branch"]].astype(np.int64).reshape(n_samples, M)
    ...
    feature_idx = [col[name] for name in feature_cols]
    X = raw[:, feature_idx].reshape(n_samples, M, -1)
    if branch_onehot:
    ...
    gaps = raw[:, col["metric_gap"]].reshape(n_samples, M).astype(np.float32)
    basin = raw[:, col["basin"]].reshape(n_samples, M).astype(np.float32)
