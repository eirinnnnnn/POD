...
def main():
    parser = argparse.ArgumentParser(description="Predict top-k branches with shared metric-gap regressor.")
    ...
    with torch.no_grad():
        for start in range(0, len(Xn), args.batch):
            ...
            B = pb.shape[0]
            scores = model(torch.from_numpy(pb).reshape(B * M, n)).reshape(B, M)
            top = torch.topk(scores, k=min(args.topk, M), dim=1, largest=False).indices.cpu().numpy()
            top_rows.append(top)
