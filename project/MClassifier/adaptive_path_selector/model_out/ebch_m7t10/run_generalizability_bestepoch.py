#!/usr/bin/env python3
"""Same as run_generalizability.py, but uses the best-epoch-selected t=64
selector (weights_t64_snr2p0_bestepoch.txt) as the single frozen
mismatched reference, evaluated at t in {8,32,64,96} -- the mismatched
half of a clean, uniform best-epoch-vs-best-epoch matched/mismatched
comparison. Writes generalizability_bestepoch.txt."""
import argparse
import csv
import subprocess
import sys
...
def main():
    ...
    if args.side == "model_out":
        ...
        selector = MO_DIR / "weights_t64_snr2p0_bestepoch.txt"
        out_txt = MO_DIR / "generalizability_bestepoch.txt"
        state_dir = MO_DIR / "data" / "generalizability_bestepoch"
    else:
        sim = BUILD / "mclass_adaptive_m_bler_sim"
        selector = PM_DIR / "weights_t64_snr2p0_bestepoch.txt"
        out_txt = PM_DIR / "generalizability_bestepoch.txt"
        state_dir = PM_DIR / "data" / "generalizability_bestepoch"
    state_dir.mkdir(parents=True, exist_ok=True)

    for t in T_LIST:
        for i, snr in enumerate(SNR_LIST):
            if already_done(out_txt, t, snr):
                print(f"{args.side} t={t} SNR={snr}: already in generalizability_bestepoch.txt, skipping", flush=True)
                continue
            cs = 1305001 + t * 10 + i
            ms = 1405001 + t * 10 + i
            ...
            print(f"=== {args.side} t={t} SNR={snr} (bestepoch t=64 mismatched) ===", flush=True)
    ...
    print(f"GENERALIZABILITY_BESTEPOCH_{args.side.upper()}_DONE", flush=True)
