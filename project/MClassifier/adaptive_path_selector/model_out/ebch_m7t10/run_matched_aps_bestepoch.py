#!/usr/bin/env python3
"""Same as run_matched_aps.py, but uses the best-epoch-selected selectors
(weights_t{T}_snr2p0_bestepoch.txt) instead of the original final-epoch
ones, to test whether best-epoch model selection alone (no input/output
reformulation) fixes the matched-losing-to-mismatched pattern seen with
the final-epoch weights. Writes matched_aps_bestepoch.txt."""
import argparse
import csv
import subprocess
import sys
...
def main():
    ...
    if args.side == "model_out":
        ...
        out_txt = MO_DIR / "matched_aps_bestepoch.txt"
        state_dir = MO_DIR / "data" / "matched_aps_bestepoch"
    else:
        sim = BUILD / "mclass_adaptive_m_bler_sim"
        out_txt = PM_DIR / "matched_aps_bestepoch.txt"
        state_dir = PM_DIR / "data" / "matched_aps_bestepoch"
    state_dir.mkdir(parents=True, exist_ok=True)

    for t in T_LIST:
        selector = (MO_DIR if args.side == "model_out" else PM_DIR) / f"weights_t{t}_snr2p0_bestepoch.txt"
        for i, snr in enumerate(SNR_LIST):
            if already_done(out_txt, t, snr):
                print(f"{args.side} t={t} SNR={snr}: already in matched_aps_bestepoch.txt, skipping", flush=True)
                continue
            cs = 1205001 + t * 10 + i
            ms = 1105001 + t * 10 + i
            ...
            print(f"=== {args.side} t={t} SNR={snr} (bestepoch matched selector) ===", flush=True)
    ...
    print(f"MATCHED_APS_BESTEPOCH_{args.side.upper()}_DONE", flush=True)
