#!/usr/bin/env python3
"""ONE combined panel, pm and model_out overlaid directly (not
side-by-side), from generalizability.txt (both sides). Same color per
checkpoint t, solid=model_out vs dashed=pm, so the gap between a matched
pair at the same t is exactly the model_out-vs-pm gain, read directly off
the plot. Saved as csv.png."""
import csv
...
PM_RESULTS = HERE.parent.parent / "pm" / "ebch_m7t10" / "cross_t_stream_results.txt"
...
def main():
    ...
    ax.set_title("model_out vs pm generalization -- overlaid (solid=model_out, dashed=pm)")
