#!/usr/bin/env python3
"""Cross-checkpoint generalization test for model_out, streaming version:
mclass_adaptive_m_bler_sim_model_out computes mean_m/BLER directly (decode
-> trace model -> frozen t=64 adaptive-m selector -> pickBest -> check
...
SELECTOR = HERE / "weights_t64_snr2p0.txt"
...
def main():
    ...
    for t, model_path in TRACE_MODELS.items():
        for i, (snr, cap) in enumerate(SNR_SAMPLES):
            ...
            cmd = [str(SIM), "-ini", str(INI), "--snr", str(snr), "--samples", str(cap),
                   ...
                   "--trace-model", str(model_path), "--selector-model", str(SELECTOR),
    ...
    print("STREAM_MODEL_OUT_DONE", flush=True)
