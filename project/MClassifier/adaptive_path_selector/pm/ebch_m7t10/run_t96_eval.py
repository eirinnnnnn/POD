...
OUTDIR = Path("/home/eirin/Polar_eirin_20260107/project/MClassifier/adaptive_path_selector/pm/ebch_m7t10/data/cross_t")
...
for i, (snr, samples) in enumerate(SNR_SAMPLES):
    ...
    cmd = [str(SIM), "-ini", str(INI), "--snr", str(snr), "--samples", str(samples),
           "--target-errors", "100",
           "--checkpoint", str(t), "--channel-seed", str(cs), "--message-seed", str(ms),
           "--out", str(out)]
    print(f"=== pm t=96 SNR={snr} ===", flush=True)
