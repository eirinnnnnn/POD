#!/usr/bin/env python3
"""pm variant of the per-prefix-length BCE reformulation of the
...
Input/target come from the same CSV as train_adaptive_m.py:
    ...
    any_basin_exists, t{T}_pm_min_sorted_1..M

Run with the log1p(pm_min) input transform ON by default (raw pm_min is
an unbounded, heavy-tailed linear metric) -- same reasoning as
train_adaptive_m.py's pm variant. Use --no-log-input to disable.
"""
import argparse
import copy
