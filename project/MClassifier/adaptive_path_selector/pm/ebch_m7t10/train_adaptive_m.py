#!/usr/bin/env python3
"""Train the adaptive path selector: a SET-LEVEL MLP regressor that takes
the static-pm ranker's own sorted (ascending) per-branch checkpoint pm_min
vector (dimension M) and predicts m_required -- the smallest pruning width
that is guaranteed to include the branch full-PED itself would pick (see
NOTES.md for the exact definition and why it deliberately does not match
static_bler(k)).

Input/target come straight from mclass_adaptive_m_dataset's CSV:
    sample, M, m_required, full_ped_correct, t{T}_pm_min_sorted_1..M

Self-contained: trains AND exports a plain-text weight file in one step
(no separate export script), since this model's input/output shape doesn't
match architectures/trace/export_model_weights.py's per-branch-ranker
checkpoint format.
"""
import argparse
...
def load_dataset(path):
    ...
    M = int(raw[0, col["M"]])
    feature_cols = [n for n in names if n.startswith("t") and "_pm_min_sorted_" in n]
    feature_cols.sort(key=lambda n: int(n.rsplit("_", 1)[1]))
    if len(feature_cols) != M:
        raise ValueError(f"expected {M} sorted-pm_min columns, found {len(feature_cols)}")
...
def log_transform_inputs(X):
    # pm_min is a raw, linear accumulated SCL path metric (a sum of |LLR|
    # penalties, never itself log-transformed anywhere in this codebase) --
    # log1p is safe since pm_min >= 0 always, and compresses its heavy
    # right tail before standardization.
    return np.log1p(X)
