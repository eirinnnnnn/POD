...
int main(int argc, char const *argv[]){
    ...
    config["BSC"]["step_type"] = "p";
    config["BSC"]["start"] = "0.01";
    config["BSC"]["step"] = "0.01";
    config["BSC"]["end"] = "0.15";
    config["BSC"]["step_mode"] = "additive";
    config["BSC"]["seed_string"] = "-2";
    ...
    channelBase *channel_BSC = new BSC(config["BSC"]);
    channel_BSC->setCodeRate(ECC_APD->getCodeRate());
    ...
    std::string ini_seed_string = channel_BSC->getSeedString();
    ...
    while(do_monte_carlo){
        ...
        // channel
        channel_BSC->addNoise(codeword,received);
        ...
        if(iter_count % monitor_slot_size == 0){
            printf("%s, BLER = %8d/%8d = %8.6f, BER = %8.6f, RBER = %8.6f\r",
                channel_BSC->getChannelInfo().c_str(),
        ...
        || (iter_count >= iter_min && block_error_count >= error_max)){
            ...
            fprintf(log_data,"%s, BLER = %8d/%8d = %8.6f\n",channel_BSC->getChannelInfo().c_str(),
                                                            ...
                                                            (double)block_error_count/iter_count);
            fclose(log_data);
            do_monte_carlo = (channel_BSC->nextChannel() ? true:false);
            channel_BSC->setSeedString(ini_seed_string);
