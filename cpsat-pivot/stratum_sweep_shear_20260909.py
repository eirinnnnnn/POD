...
CODES = {                                   # name -> (generator, P, m)
    ...
    # anchored on the z1z3-shear permutation (Z-sum 2.09497 @ 3dB, a verified
    # reachable improvement over the equation anchor's 2.31602).  Same stratum
    # family; forward search below the NEW, lower B_T.
    "m6_t11_shear": ("eBCH_m6_t11.matrix", "eBCH_m6_t11_P_shear_z1z3.matrix", 6),
...
def main() -> int:
    ...
    # share the base code's cache: every INFEASIBLE below the old anchor is
    # still INFEASIBLE, and the shear sweep only tightens B_T.
    cache_code = args.code[:-6] if args.code.endswith("_shear") else args.code
    cache_path = os.path.join(HERE, f"stratum_sweep_{cache_code}_cache.json")
