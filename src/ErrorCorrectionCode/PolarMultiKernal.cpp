...
bool PolarMultiKernal::doDecode(std::vector<double> &received, std::vector<char> &decoded_word, std::string &log){
    ...
    for(unsigned int codeword_idx=0; codeword_idx<received.size(); codeword_idx++){
        if (KM->bha_value_type[received_order[codeword_idx]] == "?")
            SCL_mem[0][stage][received_order[codeword_idx]].value = received[codeword_idx]; // need to change to LLR domain
        else if (KM->bha_value_type[received_order[codeword_idx]] == "1")
            SCL_mem[0][stage][received_order[codeword_idx]].value = 99999999999999999.9999;
        else if (KM->bha_value_type[received_order[codeword_idx]] == "0")
            SCL_mem[0][stage][received_order[codeword_idx]].value = 0.0;
        else
            ERROR("bha_value_setting[%d]=%s", codeword_idx, KM->bha_value_type[received_order[codeword_idx]]);
    }
    list_enable[0] = 1;
