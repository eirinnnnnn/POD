...
bool PolarMultiKernal::decodeOnceWithOrderTrace(const std::vector<double> &received,
                                          const std::vector<unsigned int> &order,
                                          std::vector<char> &decoded_word,
                                          double &best_metric,
                                          std::string &log,
...
{
    (void)log;
    checkpoint_trace.assign(checkpoints.size(), PSPEDTracePoint());
    ...
    // main SCL loop, with a checkpoint snapshot of the running best
    // path_metric among currently-enabled paths taken after each requested
    // decode_idx position (checkpoints are 1-based progress counts, e.g.
    // checkpoint=6 means "after processing decode_idx 0..5")
    unsigned int cp_pos = 0;
    for(unsigned int decode_idx=0; decode_idx<codeword_length; decode_idx++){
        for(unsigned int list_idx=0; list_idx<list_size; list_idx++)
            if(list_enable[list_idx])
                KM->do_node_value(decode_idx, 0, SCL_mem[list_idx]);
        ...
        while(cp_pos < checkpoints.size() && checkpoints[cp_pos] == decode_idx+1){
            PSPEDTracePoint tp;
            double best1 = 1e100, best2 = 1e100, sum = 0.0, worst = -1e100;
            for(unsigned int list_idx=0; list_idx<list_size; list_idx++){
                if(!list_enable[list_idx]) continue;
                double pm = path_metric[list_idx];
                tp.active++;
                sum += pm;
                if(pm > worst) worst = pm;
                if(pm < best1){ best2 = best1; best1 = pm; }
                else if(pm < best2){ best2 = pm; }
            }
            if(tp.active > 0){
                tp.pm_min = best1;
                tp.pm_max = worst;
                tp.pm_mean = sum / tp.active;
                tp.pm_gap = (tp.active >= 2) ? (best2 - best1) : 0.0;
            }

            // llr_abs_*: raw channel-input magnitude over codeword
            // positions already consumed (order[i] < checkpoint), same
            // convention as the eBCH side -- decoder-agnostic, needs only
            // `received` and the leaf-order map.
            {
                unsigned int upto = decode_idx + 1;
                double abs_sum = 0.0, abs_min = 1e100, abs_max = 0.0;
                unsigned int used = 0;
                for(unsigned int i=0; i<received.size(); i++){
                    if(order[i] >= upto) continue;
                    double v = ABS(received[i]);
                    abs_sum += v;
                ...
                if(used > 0){
                    tp.llr_abs_min = abs_min;
                    tp.llr_abs_mean = abs_sum / used;
                    tp.llr_abs_max = abs_max;
                }
            }

            checkpoint_trace[cp_pos] = tp;
            cp_pos++;
        }
    }
...
bool PolarMultiKernal::decodeAllBranchesTrace(const std::vector<double> &received,
                                          ...
                                          std::string &log){
    if(!useAED){
        candidates.assign(1, std::vector<char>(received.size(), 0));
        metrics.assign(1, 0.0);
        valid.assign(1, 0);
        checkpoint_trace.assign(1, std::vector<PSPEDTracePoint>());
        bool ok = decodeOnceWithOrderTrace(received, received_order, candidates[0], metrics[0], log,
                                            checkpoints, checkpoint_trace[0]);
        if(!ok) metrics[0] = 1e100;
        valid[0] = (char)(ok && parityCheckZero(candidates[0]));
        return ok;
    }

    const unsigned int L = (aed_L == 0) ? (unsigned int)received_order_set.size() : aed_L;
    candidates.assign(L, std::vector<char>(received.size(), 0));
    metrics.assign(L, 0.0);
    valid.assign(L, 0);
    checkpoint_trace.assign(L, std::vector<PSPEDTracePoint>());

    bool have_any = false;
    for(unsigned int ell=0; ell<L; ell++){
        double metric = 0;
        bool ok = decodeOnceWithOrderTrace(received, received_order_set[ell], candidates[ell], metric, log,
                                            checkpoints, checkpoint_trace[ell]);
        metrics[ell] = ok ? metric : 1e100;
        valid[ell] = (char)(ok && parityCheckZero(candidates[ell]));
        have_any = have_any || ok;
    }
    return have_any;
}
